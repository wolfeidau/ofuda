...
Ofuda.prototype.validateHttpRequestSync = function (request, authCallback) {
    ...
    if (null !== accessKeyTokens) {
        ...
        if (_.isObject(credentials)) {
            ...
            throw new Error('auth callback should return an object');
        }
    }
...
Ofuda.prototype.validateHttpRequest = function (request, authCallback, callback) {
    if (arguments.length == 2) return this.validateHttpRequestSync(request, authCallback); // backwards compat
    var self = this,
    ...
    if (null !== accessKeyTokens) {
        var accessKeyId = accessKeyTokens[0],
            suppliedSignature = accessKeyTokens[1],
            asyncAuth = function(credentials, _passedcallback) {
                var thiscallback = typeof _passedcallback == 'function' ? _passedcallback : callback;
        ...
        if (typeof(authCallbackResponse) == 'object') { // authCallback returned sync
            asyncAuth(authCallbackResponse, callback);
        } else if (typeof(authCallbackResponse) != 'undefined') {
            throw new Error('auth callback should return an object');
        }
    } else {
        callback(false);
